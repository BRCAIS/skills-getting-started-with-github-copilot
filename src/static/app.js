          ...
          <div class="participants-section">
            ...
            ${details.participants.length > 0
              ? `<ul>${details.participants
                  .map(
                    (participant) =>
                      `<li class="participant-row"><span>${participant}</span><button type="button" class="unregister-button" data-activity="${name}" data-email="${participant}" aria-label="Unregister ${participant} from ${name}" title="Unregister participant">&#128465;</button></li>`
                  )
                  .join("")}</ul>`
